<template lang="pug">
    .films-home.section-padding(
        :class="phone ? 'pt-4' : ''"
    )
        v-container
            header1(
                center
            ) Najpopularniejsze filmy

            //- laptop mockup
            v-row.justify-center(
                :class="phone ? 'pt-3' : 'pt-12'"
                v-if="videos.length"
            )

                v-col(
                    :cols="phone ? 12 : tablet ? 10 : laptop ? 9 : desktop ? 7 : 8"
                    :class="phone ? 'mb-0' : 'mb-7'"
                )
                    .laptop

                        film-big-timg.z-index-5(
                            :video="videos[0]"
                            @click="openDialog"
                        )

                        img.blob-1(
                            src="/images/blob-1.png"
                            :height="phone ? 150 : tablet ? 250 : desktop ? 280 : 320"
                        )


            //- film tiles
            v-row(
                style="position: relative;"
                :class="phone ? 'pb-8' : 'pb-10'"
                v-if="videos.length"
            )
                img.blob-2(
                    src="/images/blob-2.png"
                    :height="phone ? 70 : tablet ? 130 : laptop ? 160 : 200"

                )

                v-col.z-index-5(
                    cols="4"
                    v-for="(video, index) in $options.filters.chunkItems(videos, 2, 3)"
                    :key="index"
                    :class="phone ? 'px-1' : ''"
                )
                    film-timg(
                        @click="openDialog"
                        :video="video"
                    )

            //- button
            v-row
                v-col.d-flex.justify-center
                    v-btn.font-weight-bold.px-7.py-4(
                        rounded
                        color="secondary"
                        depressed
                        small
                        to="/filmy"
                    ) Zobacz wszystkie


            //- popup
            v-dialog(
                v-model="dialog"
                @click:outside="stopVideo"
                :width="tablet ? '90%' : '70%'"
                overlay-opacity=".8"
            )
                v-responsive(
                    :aspect-ratio="16/9"
                    width="100%"
                )
                    youtube(
                        :video-id="videoID"
                        ref="youtube"
                        width="100%"
                        height="100%"
                    )
</template>

<script>
import { mapState } from "vuex";

import Header1 from "@/components/molecules/Header1";
import FilmBigTimg from "@/components/molecules/FilmBigTimg";
import FilmTimg from "@/components/molecules/FilmTimg";

export default {
    name: "films-home",

    components: {
        Header1,
        FilmTimg,
        FilmBigTimg
    },

    data: () => ({
        dialog: false,
        videoID: null
    }),

    computed: {
        player() {
            return this.$refs.youtube.player;
        },
        ...mapState(["videos"])
    },

    methods: {
        stopVideo() {
            this.player.stopVideo();
        },
        playVideo() {
            this.player.playVideo();
        },
        openDialog(val) {
            this.dialog = true;
            this.videoID = val.acf.id;
        }
    }
};
</script>

<style lang="sass" scoped>
.films-home
    .blob-1
        position: absolute
        left: -45px
        top: -55px

        @media(max-width: 959.58px)
            top: -45px

        @media( max-width: 599.98px )
            left: -10px
            top: -35px

    .blob-2
        position: absolute
        right: -60px
        top: -65px

        @media(max-width: 959.58px)
            right: -10px
            top: 100px
            transform: rotate(230deg)

        @media( max-width: 599.98px )
            top: 140px
            right: -20px

    .laptop
        width: 100%
        position: relative

        .laptop-img
            width: 100%
            height: 100%
            object-fit: contain
</style>
